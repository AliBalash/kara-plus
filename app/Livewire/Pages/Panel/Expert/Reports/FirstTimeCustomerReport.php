...
class FirstTimeCustomerReport extends Component
{
    ...
    {
        return route('reports.first-time-customers.export', $this->exportQuery());
    }

    public function render()
    {
        $report = app(OperationsReportService::class)->firstTimeCustomers($this->filters());
        $rows = $this->paginateRows($report['rows'], $this->perPage);

        return view('livewire.pages.panel.expert.reports.first-time-customer-report', [
            'report' => $report,
            'rows' => $rows,
